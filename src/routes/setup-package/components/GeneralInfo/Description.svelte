...
{#if state.peneranganTab === 'description'}
	<section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
		...
		<div class="flex flex-col p-6">
			{#each state.descriptions as desc, i}
				<div class="mb-6 rounded-lg border border-gray-200 bg-gray-50/50 p-4">
					<div class="mb-4 flex items-center justify-between">
						...
						<button
							...
							onclick={() =>
								(state.descriptions = state.descriptions.filter((_, idx) => idx !== i))}
							><X size={16} /></button
						>
					</div>
			...
			<button
				...
				onclick={() => (state.descriptions = [...state.descriptions, { image: '', text: '' }])}
			>
...
{#if state.peneranganTab === 'audience'}
	<section class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
		...
		<div class="flex flex-col gap-3 p-6">
			{#each state.targetAudience as audience, i}
				<div class="flex items-center gap-3">
					<input
						type="text"
						placeholder="e.g., First-time Umrah pilgrims"
						bind:value={state.targetAudience[i]}
					...
					<button
						class="text-gray-400 hover:text-red-500"
						onclick={() =>
							(state.targetAudience = state.targetAudience.filter((_, idx) => idx !== i))}
						><X size={18} /></button
					>
				</div>
			{/each}
			<button
				...
				onclick={() => (state.targetAudience = [...state.targetAudience, ''])}
			>
