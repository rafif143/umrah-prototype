<script>
	...
	// Local convenience variables mapped to state methods/properties
	// We access state directly in the template, but for event handlers sometimes it's easier to alias
</script>
	...
	<div class="border-b border-gray-100 bg-gray-50/50 px-6 py-4">
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="mt-0.5 text-xs text-gray-500">Manage hotel accommodations.</p>
			</div>
			<button
				...
				onclick={() => (state.showAccommodationModal = true)}
			>
				<Plus size={16} />
				Add Accommodation
			</button>
		...
		<div class="overflow-hidden rounded-lg border border-gray-200">
			<table class="w-full text-left text-sm">
				...
				<tbody class="divide-y divide-gray-200">
					{#each state.accommodationList as item, index}
					...
					{/each}
					{#if state.accommodationList.length === 0}
						<tr>
