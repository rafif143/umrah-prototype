<script>
	import { X, ChevronDown, User, FileText, Globe } from 'lucide-svelte';
	...
	let mockGuests = $derived.by(() => {
		if (!show || !room || !isOccupied) return [];
		const type =
			typeOptions.find((t) => t.value === selectedType) ||
			typeOptions.find((t) => t.value === room.type);
		const count = type ? type.cap : 0;
		...
		>
			...
			>
				<div>
					...
					<p class="mt-0.5 text-xs text-gray-500">
						...
						{#if room?.originalType !== selectedType}
							<span class="font-medium text-orange-500">→ {selectedType?.toUpperCase()}</span>
						{/if}
					</p>
				</div>
				<button
									...
									>
										<div class="flex items-center gap-3">
											...
											<div>
												...
												<div class="text-[10px] text-gray-500">Pass: {guest.passport}</div>
											</div>
										</div>
										<ChevronDown
											size={16}
									...
									{#if openGuestIndex === i}
										...
										>
											<div class="grid grid-cols-2 gap-x-4 gap-y-3 text-xs">
												<div>
													<span class="mb-0.5 block text-gray-400">Gender</span>
													<span class="font-medium text-gray-700">{guest.gender}</span>
												</div>
												<div>
													<span class="mb-0.5 block text-gray-400">Age</span>
													<span class="font-medium text-gray-700">{guest.age} Years</span>
												</div>
												<div class="col-span-2">
													<span class="mb-0.5 block text-gray-400">Nationality</span>
				...
				<div class="border-t border-gray-100 bg-gray-50/50 p-5 pt-4">
					...
					<div class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
						<div class="mb-2 flex items-center justify-between">
							...
							<span class="text-xs font-medium text-gray-900 capitalize">{selectedType}</span>
						</div>
						...
						<p class="mt-2 text-center text-[10px] text-gray-400">
							Original: <span class="capitalize">{room?.originalType}</span>
							{#if room?.originalType !== selectedType}
								· Changed to {selectedType}
							{/if}
						</p>
					</div>
				</div>
			...
			>
				...
				<button
					...
					onclick={handleSave}
					disabled={selectedType === room?.type}
				>
					Save
				</button>
			</div>
